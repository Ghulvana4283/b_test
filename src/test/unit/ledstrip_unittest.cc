...
{
    // given
    memset(&ledStripDetailedModeConfigMutable()->ledConfigs, 0, sizeof(ledStripDetailedModeConfigMutable()->ledConfigs));
    ...
    for (uint8_t index = 0; index < WS2811_LED_STRIP_LENGTH; index++) {
        ...
        printf("iteration: %d\n", index);
#endif
        EXPECT_EQ(expectedLedStripConfig[index], ledStripDetailedModeConfig()->ledConfigs[index]);
    }
...
TEST(LedStripTest, smallestGridWithCenter)
{
    ...
    memcpy(&ledStripDetailedModeConfigMutable()->ledConfigs, &testLedConfigs, sizeof(testLedConfigs));
...
{
    // given
    memset(ledStripDetailedModeConfigMutable()->colors, 0, sizeof(hsvColor_t) * LED_CONFIGURABLE_COLOR_COUNT);
    ...
    for (uint8_t index = 0; index < TEST_COLOR_COUNT; index++) {
        ...
        EXPECT_EQ(expectedColors[index].h, ledStripDetailedModeConfig()->colors[index].h);
        EXPECT_EQ(expectedColors[index].s, ledStripDetailedModeConfig()->colors[index].s);
        EXPECT_EQ(expectedColors[index].v, ledStripDetailedModeConfig()->colors[index].v);
    }
}
